...
function PriceCard({ data, isUpdated }: { data: PriceData; isUpdated: boolean }) {
  ...
  return (
    <motion.div
      ...
      style={{
        background: isUpdated ? 'rgba(184,134,11,0.08)' : 'rgba(255,255,255,0.03)',
        border: isUpdated ? '1px solid rgba(184,134,11,0.3)' : '1px solid rgba(255,255,255,0.07)',
        backdropFilter: 'blur(12px)',
        boxShadow: isUpdated ? '0 0 20px rgba(184,134,11,0.1)' : '0 4px 24px rgba(0,0,0,0.2)',
      }}
    >
      <div
      ...
      <div className="flex items-start justify-between gap-2 relative z-10">
        <div>
          <p className="text-gray-500 text-xs uppercase tracking-widest mb-0.5">{g.asset}</p>
          <p className="text-white font-semibold text-sm leading-tight">{data.SecuritySlug}</p>
        </div>
        <div className={`flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium ${isPositive ? 'bg-emerald-400/10 text-emerald-400' : 'bg-red-400/10 text-red-400'}`}>
      ...
      <div className="relative z-10">
        <p className="text-gray-500 text-xs uppercase tracking-widest mb-1">{g.lastPrice}</p>
      ...
      <div className="grid grid-cols-2 gap-3 relative z-10">
        <div className="rounded-xl p-3" style={{ background: 'rgba(255,255,255,0.03)' }}>
          <p className="text-gray-500 text-xs mb-1 uppercase tracking-wider">{g.bid}</p>
          <p className="text-white text-sm font-medium">{formatPrice(data.Bid)}</p>
        </div>
        <div className="rounded-xl p-3" style={{ background: 'rgba(255,255,255,0.03)' }}>
          <p className="text-gray-500 text-xs mb-1 uppercase tracking-wider">{g.ask}</p>
      ...
      <div className="flex items-center justify-between text-xs relative z-10 pt-1 border-t border-white/5">
        <span className="text-gray-500">
          {g.high}: <span className="text-emerald-400">{formatPrice(data.HighPrice)}</span>
        </span>
        <span className="text-gray-500">
          {g.low}: <span className="text-red-400">{formatPrice(data.LowPrice)}</span>
        </span>
      </div>
...
export default function PricesDashboard() {
  ...
  return (
    <section id="fiyatlar" className="bg-[#030308] relative" aria-label={`${g.title} ${g.titleGradient}`}>
      <div
      ...
      <div className="max-w-7xl mx-auto px-6 py-24">
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-12">
          <div>
            <div className="text-[#7c3aed] text-xs uppercase mb-3 flex items-center gap-2" style={{ letterSpacing: '0.2em' }}>
            ...
            <p className="text-gray-500 mt-2">{g.subtitle}</p>
          </div>
        ...
        {prices.length === 0 && (
          <div className="flex flex-col items-center justify-center py-32 gap-4">
            ...
            <p className="text-gray-500">{g.loading}</p>
          </div>
        )}
        ...
        {prices.length > 0 && (
          ...
          >
            {prices.map(p => (
              <PriceCard key={p.SecuritySlug} data={p} isUpdated={p.SecuritySlug === updatedSlug} />
            ))}
          </motion.div>
        )}
      </div>
